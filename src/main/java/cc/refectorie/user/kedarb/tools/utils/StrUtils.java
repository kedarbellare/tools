...
public class StrUtils {
    ...
    public static String fmt(double v) {
        int vint = new Double(v).intValue();
        if (Math.abs(v - vint) < 1e-40) // probably an integer
            return new Integer(vint).toString();
        else if (Math.abs(v) < 1e-3) // scientific notation (close to 0)
            return String.format("%.2e", v);
        else if (Math.abs(v) > 1e3) // scientific notation (large)
            return String.format("%.2e", v);
        else
