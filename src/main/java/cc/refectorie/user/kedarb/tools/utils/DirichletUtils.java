...
public class DirichletUtils {
	...
	public static double expectedLog(double count, double totalCount) {
		...
		if (!NumUtils.isFinite(x))
			throw new RuntimeException(String.format("count=%f, totalCount=%f",
					count, totalCount));
		return x;
	}
	...
	public static double fastExpDigamma(double count) { // if count == 0, return
		// 0
